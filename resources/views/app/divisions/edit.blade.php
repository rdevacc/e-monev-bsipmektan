    ...
    <div class="py-4 md:pl-[17rem] md:pr-4">
        <div class="w-full p-4 bg-white border shadow-md rounded-md lg:max-w-full space-y-4 dark:bg-gray-800 dark:border-gray-700">
            <div class="flex px-2 pb-4 pt-4 md:px-0 justify-center">
                <h2 class="text-xl md:text-2xl font-semibold">Edit Subkelompok</h2>
            </div>
            <form action="/app/divisions/{{ $data->id }}" method="POST" class="w-full bg-white shadow-md rounded my-1 px-4 pt-2 pb-4 space-y-4 lg:px-16">
                @method('PUT')
                @csrf
                ...
                <div class="flex justify-between sm:flex-col sm:space-y-1">
                    ...
                    <input class="input-form" name="head_name" id="head_name" type="text" placeholder="Nama Kepala Subkelompok" value="{{ old('head_name') ? : $data->head_name}}">
                </div>
                <div class="flex justify-between sm:flex-col sm:space-y-1">
                    ...
                    <select name="department_id" id="department_id"  class="input-form">
                        ...
                        @foreach ($departments as $department)
                            @if (old('department_id', $data->department->id) == $department->id)
