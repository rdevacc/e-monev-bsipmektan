 ...
 <div class="h-screen flex-col justify-center items-center md:pr-4 bg-gray-800">
    ...
    <div class="w-5/6 sm:w-[80%] md:w-[50%] lg:w-[40%] xl:w-[30%] px-4 pb-4 mx-auto bg-slate-100 border border-gray-800 rounded-lg shadow dark:bg-slate-100 dark:border-gray-700">
        <form class="space-y-6" action="/app/login" method="POST">
            ...
            @enderror
            @if (session('success'))
            <div class="flex bg-green-800 rounded-md p-2 text-center justify-center items-center">
                <span class="text-slate-100 align-middle">{{ session('success') }}</span>   
            </div>
            @endif
            <div>
                <label for="email" class="block mb-2 text-sm text-gray-900 dark:text-gray-800">Your E-mail</label>
                <input autofocus type="email" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-200 dark:border-gray-500 dark:placeholder-gray-400 dark:text-gray-800" placeholder="name@domain.com" required>
            </div>
            <div>
                <label for="password" class="block mb-2 text-sm text-gray-900  dark:text-gray-800">Your Password</label>
            ...
            <div class="flex items-start">
                ...
                <a href="{{ route('forgot-password') }}" class="ml-auto text-sm text-blue-700 hover:underline dark:text-blue-500">Forgot Password?</a>
            </div>
